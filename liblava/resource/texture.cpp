...
void vk_sampler_create_info__init_base_alpha(VkSamplerCreateInfo &sampler_info) {
  sampler_info = {};
...
bool texture::create(device_ptr device, uv2 size, VkFormat format,
                     layer::list const &layers, texture_type type) {
  texture_init_info init_info;
  init_info.device = device;
  init_info.size = size;
  init_info.format = format;
  init_info.layers = &const_cast<layer::list &>(layers);
  init_info.type = type;
  return create(&init_info);
}

bool texture::create(texture_init_info *init_info) {
  auto &device = init_info->device;
  auto &size = init_info->size;
  auto &format = init_info->format;
  auto sampler_info = init_info->sampler_info;

  layers = *init_info->layers;
  type = init_info->type;
...
lava::texture::ptr lava::load_texture(string_ref filename, texture_init_info *init_info) {
  ...
  auto use_gli = extension(str(path), {"DDS", "KTX", "KMG"});
  auto use_stbi = false;

  if (!use_gli)
    use_stbi = extension(str(path), {"JPG", "PNG", "TGA", "BMP", "PSD", "GIF", "HDR", "PIC"});
  ...
  if (use_gli) {
    ...
    if (file.opened())
      ...
      data = stbi_load(str(path), &tex_width, &tex_height, &tex_channels, STBI_rgb_alpha);
    ...
    init_info->size = size;
    init_info->format = VK_FORMAT_R8G8B8A8_UNORM;
    init_info->set_layers_to_default();
    if (!texture->create(init_info))
      return nullptr;
